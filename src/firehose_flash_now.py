...
print("=" * 60)
print("  Active Firehose Live Partition Flasher")
print("=" * 60)
...
print(f"[+] Loaded Mu UEFI: {len(mu_data)} bytes -> {mu_sectors} sectors ({len(mu_padded)} bytes)")
...
print(f"[+] Loaded ESP: {len(esp_data)} bytes -> {esp_sectors} sectors ({len(esp_padded)} bytes)")
...
try:
    # 1. Flash boot_a
    flash_partition(lun=4, start_sector=114054, num_sectors=mu_sectors, data=mu_padded, name="boot_a.img")

    # 2. Flash boot_b
    flash_partition(lun=4, start_sector=458227, num_sectors=mu_sectors, data=mu_padded, name="boot_b.img")

    # 3. Flash ESP partition 12
    ...
    print(f"[+] Reboot command sent: {r_resp}")
